...
cat > /home/claude/tree-v3/app/api/discipleship-tree/my/route.ts << 'ENDOFFILE'
...
function mapMember(row: any, branchNames: Map<string, string>): TreeMemberRecord {
    const roles = (row.user_roles ?? []).map((ur: any) => ur.role?.code).filter(Boolean) as string[];
    return {
        id: row.id, fullName: row.full_name, email: row.email,
        roles, birthDate: row.birth_date, branchId: row.branch_id,
        branchName: row.branch_id ? branchNames.get(row.branch_id) : undefined,
...
export async function GET(request: NextRequest) {
    try {
        ...
        lineageLinks.forEach((l) => { memberIds.add(l.mentor_id); memberIds.add(l.disciple_id); });
        ...
        const discipleCounts: Record<string, number> = {};
        lineageLinks.forEach((l) => { discipleCounts[l.mentor_id] = (discipleCounts[l.mentor_id] ?? 0) + 1; });
